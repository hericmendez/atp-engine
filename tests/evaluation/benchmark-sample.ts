...
// Representative identity cases: same game, different game, related game, abbreviations, remakes
const IDENTITY_SAMPLE_INDICES = [0, 1, 2, 3, 4, 5, 6, 7, 10, 15, 20, 25, 30, 35, 40, 44];
const ENRICHMENT_SAMPLE_INDICES = [0, 1, 2, 3, 5, 8, 10, 15, 20, 25, 30, 35, 39];
const CLASSIFICATION_SAMPLE_INDICES = [0, 1, 2, 3, 5, 10, 15, 20, 30, 40, 49];
...
async function benchmarkIdentity(
  ...
  for (const idx of IDENTITY_SAMPLE_INDICES) {
    const c = dataset[idx];
    if (!c) continue;
...
Reason: test`;
    ...
    process.stdout.write(
      `    [${results.length}/${IDENTITY_SAMPLE_INDICES.length}] ${c.id}: ${predicted} (expected: ${c.groundTruth}) ${error ? '❌ ' + error : predicted === c.groundTruth ? '✓' : '✗'} ${latencyMs}ms\n`,
    );
  }
  return results;
}
...
async function benchmarkEnrichment(
  ...
  const results: BenchmarkResult[] = [];
  for (const idx of ENRICHMENT_SAMPLE_INDICES) {
    const c = dataset[idx];
    if (!c) continue;
...
Which value is more likely correct?`;
    ...
    process.stdout.write(
      `    [${results.length}/${ENRICHMENT_SAMPLE_INDICES.length}] ${c.id}: ${predicted} (expected: ${c.groundTruth}) ${error ? '❌ ' + error : predicted === c.groundTruth ? '✓' : '✗'} ${latencyMs}ms\n`,
    );
  }
  return results;
}
...
async function benchmarkClassification(
  ...
  const results: BenchmarkResult[] = [];
  for (const idx of CLASSIFICATION_SAMPLE_INDICES) {
    const c = dataset[idx];
    if (!c) continue;
...
Reason: no hints`;
    ...
    process.stdout.write(
      `    [${results.length}/${CLASSIFICATION_SAMPLE_INDICES.length}] ${c.id}: ${predicted} (expected: ${c.groundTruth}) ${error ? '❌ ' + error : predicted === c.groundTruth ? '✓' : '✗'} ${latencyMs}ms\n`,
    );
  }
  return results;
}
...
async function main() {
  ...
  if (!model) {
    console.error(
      'Usage: npx tsx benchmark-sample.ts --model=<model> [--think=true|false] [--op=identity|enrichment|classification|all]',
    );
    process.exit(1);
  }

  const modeLabel = think ? 'thinking' : 'default';
  console.log(`=== ${model} (${modeLabel}) - SAMPLE BENCHMARK ===`);

  const allResults: BenchmarkResult[] = [];

  if (op === 'all' || op === 'identity') {
    console.log(`  Identity (${IDENTITY_SAMPLE_INDICES.length} representative cases)...`);
    ...
    console.log(
      `    Summary: ${idSummary.correct}/${idSummary.totalCases} correct (${idSummary.accuracy.toFixed(1)}%), avg ${idSummary.avgLatencyMs.toFixed(0)}ms, P95 ${idSummary.p95LatencyMs.toFixed(0)}ms, ${idSummary.timeouts} timeouts`,
    );
    saveResults(allResults);
  }

  if (op === 'all' || op === 'enrichment') {
    console.log(`  Enrichment (${ENRICHMENT_SAMPLE_INDICES.length} representative cases)...`);
    ...
    console.log(
      `    Summary: ${enrSummary.correct}/${enrSummary.totalCases} correct (${enrSummary.accuracy.toFixed(1)}%), avg ${enrSummary.avgLatencyMs.toFixed(0)}ms, P95 ${enrSummary.p95LatencyMs.toFixed(0)}ms, ${enrSummary.timeouts} timeouts`,
    );
    saveResults(allResults);
  }
  ...
  if (op === 'all' || op === 'classification') {
    console.log(
      `  Classification (${CLASSIFICATION_SAMPLE_INDICES.length} representative cases)...`,
    );
    ...
    console.log(
      `    Summary: ${classSummary.correct}/${classSummary.totalCases} correct (${classSummary.accuracy.toFixed(1)}%), avg ${classSummary.avgLatencyMs.toFixed(0)}ms`,
    );
    saveResults(allResults);
  }
...
function saveResults(allResults: BenchmarkResult[]) {
  const resultsFile = join(RESULTS_DIR, 'model-benchmark-sample-results.json');
  ...
  const summaryFile = join(RESULTS_DIR, 'model-benchmark-sample-summary.json');
  const summaries: ModelSummary[] = [];
  const uniqueKeys = new Set(merged.map((r) => `${r.model}|${r.thinkEnabled}|${r.operation}`));
  for (const key of uniqueKeys) {
    summaries.push(
      computeSummary(merged.filter((r) => `${r.model}|${r.thinkEnabled}|${r.operation}` === key)),
    );
  }
